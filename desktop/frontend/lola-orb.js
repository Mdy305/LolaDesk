...
(function(global){
  ...
  function mount(canvas, opts={}){
    ...
    const N    = opts.particles || Math.round(S / 3.0);   // neuron count
    const LINK = baseR * 0.68;                             // synapse connect distance (wider = more connections)
    const neurons = [];
    for(let i=0;i<N;i++){
    ...
    function step(){
      ...
      for(const n of neurons){
        ...
        const wobble = Math.sin(n.wob) * (2.5 + st.energy*5 + res*8);
      ...
      if(!REDUCED && (speaking||listening) && st.levelSm>.10 && Math.random()<st.levelSm*.55 && rings.length<10){
        rings.push(speaking
          ? { r:baseR*0.45*scale, alpha:.30+st.levelSm*.45, dir: 1, w:1.2+st.levelSm*2.2 }
          : { r:baseR*1.6*scale,  alpha:.22+st.levelSm*.35, dir:-1, w:1.0+st.levelSm*1.8 });
      }

      // idle micro-rings — subtle heartbeat so she always feels alive
      if(!REDUCED && st.state==='idle' && Math.random()<0.008 && rings.length<4){
        rings.push({ r:baseR*0.5, alpha:.14, dir:1, w:0.8 });
      }
    }
    ...
    function draw(){
      ...
      // neurons
      for(const n of neurons){
        const sz = 0.8+n.z*1.8+e*1.0+res*1.4;
      ...
      for(let k=pulses.length-1;k>=0;k--){
        ...
        ctx.beginPath(); ctx.arc(x,y,2.0,0,7); ctx.fill();
        // hot white center
