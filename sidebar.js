...
(function(){
  ...
  sidebar.innerHTML = `
    <div class="logo"><div class="logo-mark">LOLA</div><div class="logo-sub">DESK</div></div>
    <nav class="nav">${navHTML}</nav>
    <a href="operations-os.html" style="margin:0 16px 20px;padding:12px;background:linear-gradient(135deg,rgba(204,255,0,.08),rgba(176,30,108,.04));border:.5px solid var(--pink-dim);border-radius:8px;display:flex;align-items:center;gap:10px;text-decoration:none">
      <div style="font-size:20px">◉</div><div><div style="font-size:12px;font-weight:600;color:var(--pink2)">Open Operations OS</div><div style="font-size:10px;color:var(--text2)">Live recovery and owner brief</div></div>
    </a>
    <a class="nav-user" href="settings.html"><div class="nav-user-av">M</div><div class="nav-user-info"><div class="nav-user-name">Meddy</div><div class="nav-user-role">Owner · MMΛ Salon</div></div></a>`;

  const mobile = document.createElement('nav');
  mobile.className = 'mobile-bar';
  const mb = [
    { id:'overview', href:'dashboard.html', label:'Home', icon:icons.overview },
    { id:'operations', href:'operations-os.html', label:'Operate', icon:icons.operations },
