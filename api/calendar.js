import { db, getTenantByPhone, getTenantBySlug, upsertClient } from './lib/db.js';
...
async function resolveTenant(body, req){
  ...
  return getTenantByPhone(body.to || req.headers['x-lola-number'] || '');
}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==='settings'){
      return res.json({ ok:true, settings:await getBookingSettings(tenant.id) });
    }
    ...
    if(action==='day'){
      ...
      const [bookings,services,staff]=await Promise.all([
        ...
        listServices(tenant.id),listStaff(tenant.id)
      ]);
      return res.json({ ok:true,date:start.toISOString().slice(0,10),bookings,services,staff });
    }
    ...
    if(action==='hold'){
      ...
      return res.json(await holdAvailability({
        ...
        startsAt:body.starts_at,channel:body.channel||'voice',conversationId:body.conversation_id||null,
    ...
    if(action==='book'){
      ...
      if(body.hold_token){
        ...
        if(!hold || hold.status!=='active' || new Date(hold.expires_at)<=new Date()){
          return res.status(200).json({ok:false,conflict:true,error:'hold_expired'});
        }
        if(hold.service_id!==serviceId || hold.staff_id!==staffId) return res.status(200).json({ok:false,error:'hold_mismatch'});
      } else {
        const held=await holdAvailability({tenantId:tenant.id,clientId,serviceId,staffId,startsAt:body.starts_at,channel:body.channel||'voice',conversationId:body.conversation_id||null,ttlSeconds:120});
      ...
      const booking=await createCanonicalBooking({
        ...
        notes:body.notes||null,source:body.channel||body.source||'lola',conversationId:body.conversation_id||null,holdId:hold.id
      });
    ...
    if(action==='reschedule'){
      ...
      const held=await holdAvailability({tenantId:tenant.id,clientId:current.client_id,serviceId:current.service_id,staffId:body.staff_id||current.staff_id,startsAt:body.starts_at,channel:body.channel||'voice',ttlSeconds:120});
      if(!held.ok) return res.status(200).json(held);
      const updated=await updateCanonicalBooking(tenant.id,current.id,{staff_id:body.staff_id||current.staff_id,start_time:held.slot.starts_at,end_time:held.slot.ends_at,status:'confirmed'},{source:body.channel||'lola',reason:'rescheduled'});
    ...
    if(action==='cancel'){
      ...
      const updated=await updateCanonicalBooking(tenant.id,body.booking_id,{status:'cancelled'},{source:body.channel||'lola',reason:body.reason||'client_request'});
