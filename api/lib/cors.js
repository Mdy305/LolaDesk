...
export function cors(req, res) {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type,Authorization,Stripe-Signature,Telnyx-Signature-Ed25519,Telnyx-Timestamp,X-Lola-Tenant');
  if (req.method === 'OPTIONS') { res.status(204).end(); return true; }
  return false;
}

// Public endpoints (widget) — same headers, but explicit for clarity.
