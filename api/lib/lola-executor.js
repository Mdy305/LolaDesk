...
const SAFE_ACTIONS=new Set(['identify_client','get_client_context','remember_client_detail','update_client','list_services','list_staff','check_availability','hold_slot','create_booking','reschedule_booking','cancel_booking','get_booking','create_follow_up','escalate']);
...
async function resolveClient(tenantId,input={},create=false){
  const c=db(); if(!c) throw new Error('database_not_configured');
  if(input.client_id){ const {data,error}=await c.from('clients').select('*').eq('tenant_id',tenantId).eq('id',input.client_id).maybeSingle(); if(error)throw error; if(data)return data; }
...
async function getBooking(tenantId,id){
  const c=db(); if(!c)throw new Error('database_not_configured'); if(!id)return null;
  const {data,error}=await c.from('bookings').select('*').eq('tenant_id',tenantId).eq('id',id).maybeSingle(); if(error)throw error; return data||null;
}
...
export async function executeLolaAction({action,tenant,input={},channel='lola',conversationId=null,actor='lola'}={}){
  ...
  try{
    ...
    if(action==='identify_client'){
      let client=await resolveClient(tenantId,input,false); if(!client&&input.create_if_missing)client=await resolveClient(tenantId,input,true); result={ok:true,found:!!client,client};
    }else if(action==='get_client_context'){
      const client=await resolveClient(tenantId,input,false); result=client?{ok:true,found:true,client,context:await crm.getClientInsights(client.id,tenantId)}:{ok:true,found:false,client:null};
    }else if(action==='remember_client_detail'){
      const client=await resolveClient(tenantId,input,true); result=client?{ok:true,client_id:client.id,memory:await crm.rememberClientDetail(client.id,tenantId,{key:input.key,value:input.value,phone:client.phone})}:{ok:false,error:'client_required'};
    }else if(action==='update_client'){
      const client=await resolveClient(tenantId,input,true); result=client?{ok:true,client:await crm.upsertClient(tenantId,{...input,id:client.id})}:{ok:false,error:'client_required'};
      ...
      result=!resolved.ok?{ok:false,needs:resolved.needs,details:resolved}:await getAvailability({tenantId,serviceId:resolved.service.id,date:input.date||input.starts_at||new Date().toISOString(),staffId:input.staff_id||resolved.staff?.id||null,limit:Number(input.limit||12)});
      ...
      result=(!resolved.ok||!resolved.staff?.id)?{ok:false,needs:resolved.needs||'staff'}:await holdAvailability({tenantId,clientId:client?.id||null,serviceId:resolved.service.id,staffId:resolved.staff.id,startsAt:input.starts_at,channel,conversationId,ttlSeconds:Number(input.ttl_seconds||300)});
    }else if(action==='create_booking'){
      const client=await resolveClient(tenantId,input,true); if(!client) result={ok:false,error:'client_required'};
      else {
        const resolved=input.service_id&&input.staff_id?{ok:true,service:{id:input.service_id},staff:{id:input.staff_id}}:await resolveBookingRequest(tenantId,{service:input.service,stylist:input.stylist||input.staff});
        if(!resolved.ok||!resolved.staff?.id) result={ok:false,needs:resolved.needs||'staff'};
          ...
          if(hold){ const services=await listServices(tenantId),service=services.find(s=>s.id===resolved.service.id); const booking=await createCanonicalBooking({tenantId,clientId:client.id,serviceId:resolved.service.id,staffId:resolved.staff.id,locationId:input.location_id||null,startTime:hold.starts_at,endTime:hold.ends_at,status:'confirmed',totalAmount:input.total_amount??service?.price??0,notes:input.notes||null,source:channel,conversationId,holdId:hold.id}); await releaseHold(tenantId,hold.hold_token,'converted'); await crm.updateClientFromConversation(client.id,tenantId,{intent:'booking_completed',channel,summary:`Booking ${booking.id} confirmed`,phone:client.phone}); result={ok:true,status:'confirmed',booking_id:booking.id,booking}; }
        }
      }
      ...
      const c=db(); const client=await resolveClient(tenantId,input,true); const {error}=await c.from('usage_events').insert({tenant_id:tenantId,kind:'human_escalation',units:1,metadata:{client_id:client?.id||null,reason:input.reason||'requested',message:input.message||null,channel,conversation_id:conversationId}}); if(error)throw error; result={ok:true,escalated:true};
    }
    await audit(tenantId,action,!!result?.ok,{channel,actor,conversation_id:conversationId,duration_ms:Date.now()-started,error:result?.error||null}); return result;
