 ...
 * Talks to Telnyx Inference by default, Anthropic as optional fallback.
 * ENV: TELNYX_API_KEY, LLM_PROVIDER ('telnyx'|'anthropic'), LLM_MODEL, ANTHROPIC_API_KEY
 */
...
async function chatTelnyx({ system, messages, maxTokens, temperature, model, jsonMode }){
  ...
  const body = {
    ...
    messages: oaiMessages, max_tokens: maxTokens, temperature
  };
...
async function chatAnthropic({ system, messages, maxTokens, temperature, model }){
  ...
  try{
    const r = await fetch(ANTHROPIC_API, {
      ...
      body: JSON.stringify({ model: model || process.env.LLM_MODEL || DEFAULT_ANTHROPIC_MODEL, max_tokens: maxTokens, temperature, system, messages })
    });
    const data = await r.json();
