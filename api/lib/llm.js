 ...
 * ════════════════════════════════════════════════════════════════
 * ARCHITECTURE: Two providers, each used for what they're best at:
 *
 *   TELNYX (Kimi-K2.6): Phone call replies, SMS replies — fast, cheap,
 *   good at short conversational responses. Already paid for via Telnyx.
 *
 *   ANTHROPIC (Claude): Marketer analysis, strategy, campaigns — complex
 *   reasoning tasks that need reliability. Kimi returns empty on long
 *   structured tasks; Claude does not. Requires Anthropic credits.
 *
 * ENV VARS:
 *   TELNYX_API_KEY      — required for voice/SMS AI
 *   ANTHROPIC_API_KEY   — required for Marketer/strategy/campaigns
 *   LLM_PROVIDER        — override: 'telnyx' | 'anthropic' (optional)
...
function provider(){
  ...
  // Default: Telnyx for everything (already paid, works for calls/SMS)
  // If ANTHROPIC_API_KEY is set but not TELNYX_API_KEY, use Anthropic
...
// Main chat function — use source hint to pick best provider
// source: 'voice' | 'sms' → Telnyx (fast, cheap)
// source: 'marketer' | 'strategy' | 'campaign' → Anthropic (reliable for complex tasks)
...
function pickProvider(source){
  // Complex reasoning tasks: use Anthropic if available
  if(['marketer','strategy','campaign','analysis'].includes(source)){
    if(process.env.ANTHROPIC_API_KEY) return 'anthropic';
  }
  // Voice/SMS and everything else: Telnyx
  if(process.env.TELNYX_API_KEY) return 'telnyx';
  if(process.env.ANTHROPIC_API_KEY) return 'anthropic';
  return 'telnyx';
}
  ...
  for(let i=0;i<budgets.length;i++){
    try{
      ...
      if(res.httpOk && res.text && res.text.trim()){
        return { ok:true, text:res.text, raw:res.raw, provider:'telnyx', model:m, attempt:i+1 };
      }
      last = { error:res.error||'empty', raw:res.raw };
      if(!res.httpOk && res.error && !/empty/i.test(res.error)) break;
...
async function chatAnthropic({ system, messages, maxTokens, temperature, model }){
  if(!process.env.ANTHROPIC_API_KEY) return { ok:false, text:'', provider:'anthropic', error:'Missing ANTHROPIC_API_KEY — add Anthropic credits at console.anthropic.com to use Marketer and strategy features' };
