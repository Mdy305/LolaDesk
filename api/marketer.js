 ...
 *
 * All three call Claude (claude-sonnet-4-6). The frontend at /marketer
 * uses these to power the workspace.
 *
 * ENV VARS:
 *   ANTHROPIC_API_KEY
 */
...
// Helper: fetch a URL's HTML and strip down to readable text for Claude
async function fetchSite(url){
  try{
    ...
    // crude readability: strip script/style, collapse whitespace, cap length
    const text = html
      ...
      .trim()
      .slice(0, 8000);
    // also grab title and meta description
...
async function claude(system, userMsg, maxTokens=1200){
...
"""`;
  ...
  // try to parse JSON; if model wrapped it in fences, strip them
  const cleaned = raw.replace(/```json|```/g,'').trim();
  let parsed = null;
  try{ parsed = JSON.parse(cleaned); }catch{
    return { ok:true, url:site.url, title:site.title, raw, parsed:null };
  }
  return { ok:true, url:site.url, title:site.title, ...parsed };
}
...
Site analysis: ${JSON.stringify(analysis||{})}`;
  ...
  const raw = await claude(system, user, 1800);
  const cleaned = raw.replace(/```json|```/g,'').trim();
  try{ return { ok:true, ...JSON.parse(cleaned) }; }
